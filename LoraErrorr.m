function [BER_COH_AWGN_Theory ,PER_COH_AWGN_Theory , SER_COH_AWGN_Theory]=LoraErrorr(SF,BW,SINR)
...
%% Initiate SNR
%SNR = -30 : 30 ;
EbNo = SINR_Theory + 10.*log10(M/SF) ;
...
%SNR_theory = linspace(-30,30,1e3) ;
 EbNo_theory = SINR_Theory + 10.*log10(M/SF) ;
% EsNo_theory = EbNo_theory + 10.*log10(SF) ;
% Theoretical BER Calculations
%BER_nCOH_AWGN_Theory = 0.5.*qfunc(sqrt(10.^(SINR_Theory./10).*2.*M) - sqrt(1.386.*SF+1.154)) ; % T. Elshabrawy and J. Robert, "Closed-Form Approximation of LoRa Modulation BER Performance," in IEEE Communications Letters, vol. 22, no. 9, pp. 1778-1781, Sept. 2018, doi: 10.1109/LCOMM.2018.2849718.
